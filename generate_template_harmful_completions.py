#!/usr/bin/env python3
"""
Generate completions from template-formatted harmful instructions and evaluate with DeepSeek ASR.
Based on calculate_template_harmful_components.py but focused on generation and evaluation.
"""
...
def load_harmful_instructions(model_path: str, cfg: Config, n_samples: int = 0) -> Tuple[object, List[str]]:
    """
    Setup model and load harmful instructions.
...
def create_completion_dicts(original_instructions: List[str], 
    ...
    for i, (original, formatted, completion) in enumerate(zip(
        ...
        completion_dict = {
            ...
            "category": "template_generated",  # Default category
        }
...
def save_results(completion_dicts: List[Dict], evaluation: Dict, output_dir: str, 
    ...
    json_path = os.path.join(output_dir, f'template_harmful_completions_with_asr_{length_suffix}.json')
    with open(json_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"Saved results to {json_path}")
    
    # Save evaluation summary
    summary_path = os.path.join(output_dir, f'harmful_asr_summary_{length_suffix}.json')
    summary = {
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate template harmful completions and evaluate ASR with DeepSeek")
    ...
    print("Starting template-based harmful completion generation and ASR evaluation...")
    ...
    # Load model and harmful instructions
    model_base, harmful_instructions = load_harmful_instructions(
        args.model_path, cfg, args.n_samples
    )
    
    print(f"Processing {len(harmful_instructions)} harmful instructions")
    
    # Format instructions with template
    formatted_instructions = format_instructions_with_template(harmful_instructions, args.template_file)
    ...
    completion_dicts = create_completion_dicts(
        harmful_instructions, formatted_instructions, completions
    )
    ...
    if "deepseekv31_success_rate" in evaluation:
        ...
        print(f"\nDeepSeek ASR: {asr:.3f} ({asr*100:.1f}%)")
    else:
    ...
    print("\n=== Harmful Generation and Evaluation Complete ===")
